...
def _input_text_to_code_with_retry(log_path, api_key, model=config.get('model', 'gpt-3.5')):
    print("    trying ...")
    try:
        if "mistral" in model:
...
def input_text_to_code(input_text, output_path, api_key, model=config.get('model', 'gpt-3.5')):
...
def input_text_to_code_plus(input_text, output_path, api_key, model=config.get('model', 'gpt-3.5')):
    import sys
...
def generate_code(session_id, input_wav, input_text, api_key, model=config.get('model', 'gpt-3.5'), mode="basic"):
...
def full_steps(session_id, input_wav, input_text, api_key, mode, model=config.get('model', 'gpt-3.5')):
